...
<body
    ...
    <div class="flex flex-1 w-full h-screen overflow-hidden">
        ...
        <div class="hidden lg:flex w-1/2 relative flex-col justify-between bg-background-dark p-12 text-white animate-fade">
            <div class="absolute inset-0 z-0 overflow-hidden">
                ...
                <div class="absolute inset-0 bg-gradient-to-t from-background-dark via-background-dark/80 to-background-dark/40"></div>
                <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-accent/20 blur-[120px] rounded-full pointer-events-none animate-pulse"></div>
            </div>

            <div class="relative z-10 flex items-center gap-3 animate-enter delay-100">
                @if(!empty($pengaturan->logo_path))
                    <div class="h-10 w-10 flex items-center justify-center rounded-xl bg-white overflow-hidden border border-white/20 shadow-sm">
                        <img src="{{ asset('storage/' . $pengaturan->logo_path) }}" alt="Logo" class="w-full h-full object-contain p-1">
                    </div>
                @else
                    <div class="flex items-center justify-center w-10 h-10 rounded-xl bg-accent/20 text-accent border border-accent/30">
        ...
        <div class="flex w-full lg:w-1/2 flex-col justify-center items-center px-6 py-12 relative bg-background-light dark:bg-background-dark transition-colors duration-300">
            ...
            <div class="w-full max-w-[420px] flex flex-col">
                <div class="mb-8 text-center animate-enter delay-100">
                    <div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-primary/10 dark:bg-accent/10 mb-6 text-primary dark:text-accent">
                        <span class="material-symbols-outlined text-3xl">lock_open</span>
                    </div>
                    <h2 class="text-3xl lg:text-[32px] font-bold leading-tight tracking-tight dark:text-white text-slate-900 mb-3">
                        Verifikasi OTP
                    </h2>
                ...
                @if (session('error'))
                    <div class="mb-6 p-4 flex items-center gap-3 text-sm text-red-700 dark:text-red-200 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-xl animate-enter" role="alert">
                        <span class="material-symbols-outlined text-xl">error</span>
                        {{ session('error') }}
                    </div>
                @endif
                @if (session('success'))
                    <div class="mb-6 p-4 flex items-center gap-3 text-sm text-green-700 dark:text-green-200 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-xl animate-enter" role="alert">
                ...
                <form action="{{ route('otp.action', $id) }}" method="POST" class="flex flex-col gap-6 animate-enter delay-200">
                    @csrf
                    
                    <div class="flex flex-col gap-2">
                        <label class="text-base font-medium leading-normal dark:text-white text-slate-900 text-center" for="otp">
                            Kode OTP (6 Digit)
                        </label>
                        <div class="relative group input-focus-effect">
                            <input
                                ...
                                id="otp" name="otp" placeholder="••••••" type="text" inputmode="numeric" pattern="[0-9]*" maxlength="6" required autofocus autocomplete="one-time-code" />
                                
                            <div class="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 dark:text-slate-500 pointer-events-none transition-colors group-focus-within:text-primary">
                ...
                <div class="mt-8 pt-6 border-t border-gray-200 dark:border-border-dark flex flex-col items-center gap-4 w-full animate-enter delay-300">
                    <p class="text-slate-500 dark:text-gray-400 text-sm">Tidak menerima kode?</p>
                    
                    <form action="{{ route('login') }}" method="GET"> 
                       <button type="submit" class="text-primary hover:text-primary-dark dark:text-accent dark:hover:text-accent/80 font-semibold text-sm transition-colors hover:underline">
                           Kirim Ulang OTP
                       </button>
                    </form>
                    
                    <a href="{{ route('login') }}" class="mt-2 flex items-center text-sm text-slate-500 dark:text-gray-400 hover:text-slate-800 dark:hover:text-white transition-colors">
