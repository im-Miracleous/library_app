<aside id="sidebar"
    class="fixed lg:static inset-y-0 left-0 w-72 h-full bg-surface dark:bg-surface-dark border-r border-primary/20 dark:border-border-dark flex flex-col z-50 lg:z-0 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 ease-in-out shadow-2xl lg:shadow-none overflow-hidden">
    ...
    : 'flex items-center gap-3 px-4 py-3 rounded-xl text-primary-dark/80 dark:text-white/70 hover:bg-white dark:hover:bg-primary/20 hover:text-primary-dark dark:hover:text-white transition-all cursor-pointer group' }}">
            ...
            @if(Auth::user()->peran == 'admin' || Auth::user()->peran == 'owner' || Auth::user()->peran == 'petugas')
                    <div class="mt-2 mb-1 px-4 py-2 border-t border-primary/10 dark:border-white/5"></div>
                    <div
                        class="px-4 text-xs font-bold text-primary-mid/60 dark:text-white/40 uppercase tracking-widest select-none mb-2">
                        Laporan</div>
                ...
                : 'flex items-center gap-3 px-4 py-3 rounded-xl text-primary-dark/80 dark:text-white/70 hover:bg-white dark:hover:bg-primary/20 hover:text-primary-dark dark:hover:text-white transition-all cursor-pointer group' }}">
                        <span
                            class="material-symbols-outlined {{ request()->routeIs('laporan*') ? 'filled' : 'group-hover:text-primary dark:group-hover:text-accent transition-colors' }}">analytics</span>
                        <p class="text-sm {{ request()->routeIs('laporan*') ? 'font-bold' : 'font-medium' }}">Laporan & Analisis</p>
                    </a>
            @endif
